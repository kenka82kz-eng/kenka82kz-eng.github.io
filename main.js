...
(function () {
  function _fillYears() {
    const sel = document.getElementById('lf_year');
  ...
  window.openLeadModal = function (label) {
    const modal = document.getElementById('leadModal');
    const title = document.getElementById('leadModalTitle');
    if (title) title.textContent = label || 'Кеңес аламын';
    _fillYears();
  ...
  window.submitLeadForm = function (e) {
    ...
    const company = (document.getElementById('lf_company').value || '').trim();
    const revenue =  document.getElementById('lf_revenue').value;
    const year    =  document.getElementById('lf_year').value;
    const link    = (document.getElementById('lf_link').value    || '').trim();
    const problem = (document.getElementById('lf_problem').value || '').trim();
    const title   = (document.getElementById('leadModalTitle') || {}).textContent || '';

    const msg = [
      '🤝 *SAMGA-NIITS — Жаңа өтінім*',
      '📋 Мақсат: ' + title,
      '',
      '👤 ФИО: ' + name,
      '🏢 Компания: ' + (company || '—'),
      '💰 Жылдық айналым: ' + revenue,
      '📅 Ашылған жыл: ' + year,
      '🌐 Сайт/Instagram: ' + (link || '—'),
      '',
      '❓ Мәселе:',
    ...
    const btn = document.getElementById('leadSubmitBtn');
    if (btn) {
      btn.textContent = '✓ Жіберілді! WhatsApp ашылуда...';
      btn.style.background = 'linear-gradient(135deg,#059669,#047857)';
    }

    window.open('https://wa.me/77717805442?text=' + encodeURIComponent(msg), '_blank');

    setTimeout(function () { window.closeLeadModal(); }, 1400);
  };
